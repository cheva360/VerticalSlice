Shader "Custom/CelShadeURPReplenishFade"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float elapsed = (_IsFade > 0.5) ? (_Time.y - _StartTime) : 0.0;
				float t = saturate(elapsed / max(_Duration, 0.0001));
		...
		Pass
		{
			...
			float4 frag_outline(Varyings_O IN) : SV_Target
			{
				...
				return float4(_OutlineColor.rgb, _OutlineColor.a * ComputeFadeAlpha());
			}
			ENDHLSL
		}
